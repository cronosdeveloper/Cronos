...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Cronos
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Cronos
// Mac: ~/Library/Application Support/Cronos
// Unix: ~/.cronos
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cronos";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cronos.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cronos.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cronos.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cronosd.pid"));
