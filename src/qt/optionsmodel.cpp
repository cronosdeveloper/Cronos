...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CRSX);
    ...
    if (!settings.contains("nAnonymizeCrsAmount"))
        settings.setValue("nAnonymizeCrsAmount", 1000);

    nAnonymizeCrsAmount = settings.value("nAnonymizeCrsAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCrsAmount"))
        SoftSetArg("-anonymizeCrsamount", settings.value("nAnonymizeCrsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cronos.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCrsAmount:
            return QVariant(nAnonymizeCrsAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCrsAmount:
            nAnonymizeCrsAmount = value.toInt();
            settings.setValue("nAnonymizeCrsAmount", nAnonymizeCrsAmount);
            emit anonymizeCrsAmountChanged(nAnonymizeCrsAmount);
            break;
        case CoinControlFeatures:
